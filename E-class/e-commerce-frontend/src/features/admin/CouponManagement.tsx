...
const CouponManagementPage = () => {
  ...
  const handleSubmit = async (values: CouponRequest) => {
    try {
      if (editingCoupon) {
        await couponService.update(editingCoupon.id, values);
  ...
  return (
    <>
      ...
      <ModalForm
        ...
        initialValues={editingCoupon || { isActive: true, discountType: 'FIXED_AMOUNT', usageLimit: 1 }}
        modalProps={{ destroyOnClose: true, onCancel: () => setEditingCoupon(null) }}
      >
        <ProFormText name="code" label="Mã giảm giá" rules={[{ required: true }]} />
        <ProFormSelect
          name="discountType"
          label="Loại giảm giá"
          options={[
        ...
        <ProFormDigit name="usageLimit" label="Giới hạn lượt sử dụng" rules={[{ required: true }]} />
